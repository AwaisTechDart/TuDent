...
export class AboutComponent implements OnInit {
  ...
  ngOnInit() {
    if(window.innerWidth < 998){
      $('#element1').css('opacity',1).css('transform', 'translate(0)');
    }
  }
  ...
  scrolling(){
    ...
    if (remPos_small2 < 600 || remPos_small2 < -280) {
      $('#element2').css('opacity',1).css('transform', 'translate(0)');
    }

    //For card 3
    var remPos_small3 = $('#element3').offset().top - $(window).scrollTop();
    if (remPos_small3 < 600 || remPos_small3 < -280) {
      $('#element3').css('opacity',1).css('transform', 'translate(0)');
    }
    ...
    if (remPos_small4 < 600 || remPos_small4 < -280) {
      $('#element4').css('opacity',1).css('transform', 'translate(0)');
    }
  }
}
